...
try:
    with open('mobi.b', 'rb') as file:
        ...
        asyncio.get_event_loop().run_until_complete(
            asyncio.gather(get_last_marks(mobi, data, web_url), get_first_marks(mobi, data, web_url),
                           get_second_marks(mobi, data, web_url), get_timetables(mobi, data, web_url),
                           get_exams(mobi, data, web_url), get_homeworks(mobi, data, web_url)))
...
class LastPopup(Popup):
    def __init__(self, *args, **kwargs):
        ...
        self.width = 800
        self.height = 400
        index = mobi.last_marks_name.index(args[0].children[1].text)
            ...
            if self.first_half:
                ...
                for parent in args[0].children:
                    if not once:
                        ...
                        try:
                            index = mobi.red_names2.index(mobi.marks_avg_name[length - i])
                ...
                for parent in args[0].children:
                    if not once:
                        ...
                        try:
                            index = mobi.red_names.index(mobi.marks_avg_name[length - i])
    ...
    def on_pre_enter(self, *args):
        try:
            ...
            for i in range(len(mobi.last_marks)):
                ...
                name_label = Label(text=str(mobi.last_marks_name[i]), text_size=(600, 200), valign='center',
                                   halign='right')
                mark_label = Label(text=str(mobi.last_marks[i]), text_size=(None, 200), valign='center')
            ...
            marks_avg_label = Label(text='Srednie ocen z przedmiotow z I okresu:', bold=True, font_size=55)
            box_marks.add_widget(marks_avg_label)

            for i in range(len(mobi.marks_avg_name)):
                mark_avg_box = BoxLayout(orientation='horizontal')
                name_label = Label(text=str(mobi.marks_avg_name[i]), text_size=(600, 200), valign='center',
                                   halign='right')
                try:
                    index = mobi.red_names.index(mobi.marks_avg_name[i])
                    mark_avg_label = Label(text=str(mobi.red_marks[index]), color=[1, 0, 0, 1])
                except ValueError:
            ...
            timetable.add_widget(Label(text=f'[b]{mobi.lessons[2]}[/b]', markup=True, font_size=55))
